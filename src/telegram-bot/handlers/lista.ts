...
export async function handleSet({ db, telegram, chatIdStr }: HandlerDeps, codes: string[]) {
  ...
  if (removed.length > 0) {
    const createdAt = upsertTelegramPendingAction(db, chatIdStr, { kind: 'set', codes: codes.map((c) => c.toUpperCase()) });
...
export async function handleRemove({ db, telegram, chatIdStr }: HandlerDeps, codes: string[]) {
  ...
  if (toRemove.length > 0) {
    const createdAt = upsertTelegramPendingAction(db, chatIdStr, { kind: 'remove', codes: codes.map((c) => c.toUpperCase()) });
