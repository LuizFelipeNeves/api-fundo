...
export function getOrComputeCotationStats(db: Database.Database, fundCode: string): CotationStats | null {
  const code = fundCode.toUpperCase();
  const orm = drizzle(db);

  const latestRow = orm
    .select({ maxd: sql<string | null>`max(${cotation.date_iso})` })
    .from(cotation)
    .where(eq(cotation.fund_code, code))
    .get();
  const latestDateIso = String(latestRow?.maxd || '');
  if (!latestDateIso) return null;

  const cacheRow = orm
    .select({ src: fundCotationStats.source_last_date_iso, json: fundCotationStats.data_json })
    .from(fundCotationStats)
    .where(eq(fundCotationStats.fund_code, code))
    .get();
  ...
  orm
    .insert(fundCotationStats)
    .values({
      fund_code: code,
      source_last_date_iso: latestDateIso,
      computed_at: computedAt,
      data_json: JSON.stringify(stats),
    })
    .onConflictDoUpdate({
      target: fundCotationStats.fund_code,
      set: {
        source_last_date_iso: latestDateIso,
        computed_at: computedAt,
        data_json: JSON.stringify(stats),
