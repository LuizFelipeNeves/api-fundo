...
app.openapi(
  ...
  createHandler(async (c: any) => {
    ...
    const data = await getFundDetails(code);
    if (!data) {
...
app.openapi(
  ...
  createHandler(async (c: any) => {
    ...
    const indicators = await getLatestIndicators(code);
    if (!indicators) {
      return c.json({ error: 'FII não encontrado' }, 404);
    }
...
app.openapi(
  ...
  createHandler(async (c: any) => {
    ...
    const cotations = await getCotations(code, days);
    if (!cotations) {
      return c.json({ error: 'FII não encontrado' }, 404);
    }
...
app.openapi(
  ...
  createHandler(async (c: any) => {
    ...
    const dividends = await getDividends(code);
    if (!dividends) {
      return c.json({ error: 'FII não encontrado' }, 404);
    }
...
app.openapi(
  ...
  createHandler(async (c: any) => {
    ...
    const data = await getLatestCotationsToday(code);
    if (!data) {
      return c.json({ error: 'FII não encontrado' }, 404);
    }
    return { data };
...
app.openapi(
  ...
  createHandler(async (c: any) => {
    ...
    const documents = await getDocuments(code);
    if (!documents) {
      return c.json({ error: 'FII não encontrado' }, 404);
    }
