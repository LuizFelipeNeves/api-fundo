import { parseBotCommand, type BotCommand } from '../telegram-bot/commands';
...
export async function processTelegramUpdate(update: TelegramUpdate): Promise<void> {
  ...
  const text = msg?.text || '';
  if (!msg || (!text && !callbackData)) return;

  const chatIdStr = String(msg.chat.id);
  await upsertTelegramUser(null, {
    chatId: chatIdStr,
  ...
  const deps = { db: null, telegram, chatIdStr };
  switch (cmd.kind) {
    case 'help':
